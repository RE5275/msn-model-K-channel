...
#define exp1 exp1_bk
 ...
 extern double exp1( _threadargsprotocomma_ double , double , double );
...
static int _reset;
static char *modelname = "Potassium current, calcium-dependent (BK)";
...
/*CVODE*/
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
   rate ( _threadargscomma_ v , cai ) ;
    o = o + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tauo)))*(- ( ( ( oinf ) ) / tauo ) / ( ( ( ( - 1.0 ) ) ) / tauo ) - o) ;
   }
  return 0;
}
 
double alpha ( _threadargsprotocomma_ double _lVm , double _lca ) {
   double _lalpha;
 _lalpha = abar * _lca / ( _lca + exp1 ( _threadargscomma_ k1 , d1 , _lVm ) ) ;
...
double beta ( _threadargsprotocomma_ double _lVm , double _lca ) {
   double _lbeta;
 _lbeta = bbar / ( 1.0 + _lca / exp1 ( _threadargscomma_ k2 , d2 , _lVm ) ) ;
...
double exp1 ( _threadargsprotocomma_ double _lk , double _ld , double _lVm ) {
   double _lexp1;
 _lexp1 = _lk * exp ( - 2e-3 * _ld * FARADAY * _lVm / R / ( 273.15 + celsius ) ) ;
...
static void _hoc_exp1(void) {
  ...
  _thread = _extcall_thread;
  _nt = nrn_threads;
 _r =  exp1 ( _p, _ppvar, _thread, _nt, *getarg(1) , *getarg(2) , *getarg(3) );
 hoc_retpushx(_r);
}
 
static int  rate ( _threadargsprotocomma_ double _lVm , double _lca ) {
   double _la , _lb ;
 _la = alpha ( _threadargscomma_ _lVm , _lca ) ;
   _lb = beta ( _threadargscomma_ _lVm , _lca ) ;
   tauo = 1.0 / ( _la + _lb ) ;
   oinf = _la / ( _la + _lb ) ;
    return 0; }
