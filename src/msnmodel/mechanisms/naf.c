...
#define damod _p[8]
#define damod_columnindex 8
#define maxMod _p[9]
#define maxMod_columnindex 9
#define level _p[10]
#define level_columnindex 10
#define max2 _p[11]
#define max2_columnindex 11
#define lev2 _p[12]
#define lev2_columnindex 12
#define ina _p[13]
#define ina_columnindex 13
#define gna _p[14]
#define gna_columnindex 14
#define mtau _p[15]
#define mtau_columnindex 15
#define htau _p[16]
#define htau_columnindex 16
#define m _p[17]
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "modulation_naf", _hoc_modulation,
 "rates_naf", _hoc_rates,
 0, 0
};
#define modulation modulation_naf
...
static int  rates ( _threadargsproto_ ) {
    minf = 1.0 / ( 1.0 + exp ( ( v - mVhalf ) / mSlope ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( v - hVhalf ) / hSlope ) ) ;
   mtau = 0.38 + 1.0 / ( 0.6 * exp ( ( v - ( - 58.0 ) ) / 8.0 ) + 1.8 * exp ( ( v - ( - 58.0 ) ) / ( - 35.0 ) ) ) ;
   if ( v < - 60.0 ) {
     htau = 3.4 + 0.015 * v ;
     }
   else {
     htau = 0.56 + 1.1 / ( 1.0 + exp ( ( v - ( - 48.0 ) ) / 15.0 ) ) + 1.2 / ( 1.0 + exp ( ( v - ( - 48.0 ) ) / 4.0 ) ) ;
     }
     return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   gna = gbar * m * m * m * h * modulation ( _threadargs_ ) ;
   ina = gna * ( v - ena ) ;
   }
 _current += ina;
