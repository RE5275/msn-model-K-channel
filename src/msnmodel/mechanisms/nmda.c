...
#define modulation modulation_nmda
 extern double mgblock( _threadargsprotocomma_ double );
 extern double modulation( _threadargsproto_ );
 /* declare global and static user variables */
#define ca_frac ca_frac_nmda
 double ca_frac = 0.1;
#define q q_nmda
 double q = 2;
#define tau2 tau2_nmda
 double tau2 = 231;
#define tau1 tau1_nmda
 double tau1 = 5.52;
...
static int _reset;
static char *modelname = "NMDA synapse";
...
double mgblock ( _threadargsprotocomma_ double _lVm ) {
   double _lmgblock;
 double _lmg , _lalpha , _lbeta ;
 _lmg = 1.0 ;
   _lalpha = 0.062 ;
   _lbeta = 3.57 ;
   _lmgblock = 1.0 / ( 1.0 + _lmg * exp ( - _lalpha * _lVm ) / _lbeta ) ;
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ...
   itotal = g * ( v - e ) * mgblock ( _threadargscomma_ v ) * scale_factor ;
   ical = itotal * ca_frac ;
