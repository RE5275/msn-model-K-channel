...
#define nrn_init _nrn_init__kas
#define _nrn_initial _nrn_initial__kas
#define nrn_cur _nrn_cur__kas
#define _nrn_current _nrn_current__kas
#define nrn_jacob _nrn_jacob__kas
#define nrn_state _nrn_state__kas
#define _net_receive _net_receive__kas 
#define rates rates__kas 
#define states states__kas 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_kas", _hoc_setdata,
 "modulation_kas", _hoc_modulation,
 "rates_kas", _hoc_rates,
 0, 0
};
#define modulation modulation_kas
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kas kas.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Slow A-type potassium current (Kv1.2)";
...
static int  rates ( _threadargsproto_ ) {
  ...
  _lalpha = 0.25 / ( 1.0 + exp ( ( v - 50.0 ) / ( - 20.0 ) ) ) ;
   _lbeta = 0.05 / ( 1.0 + exp ( ( v - ( - 90.0 ) ) / 35.0 ) ) ;
   _lsum = _lalpha + _lbeta ;
   minf = _lalpha / _lsum ;
   mtau = 1.0 / _lsum ;
   _lalpha = 0.0025 / ( 1.0 + exp ( ( v - ( - 95.0 ) ) / 16.0 ) ) ;
   _lbeta = 0.002 / ( 1.0 + exp ( ( v - 50.0 ) / ( - 70.0 ) ) ) ;
   _lsum = _lalpha + _lbeta ;
   hinf = a + ( _lalpha / _lsum ) * ( 1.0 - a ) ;
   htau = 1.0 / _lsum ;
     return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   gk = gbar * m * m * h * modulation ( _threadargs_ ) ;
   ik = gk * ( v - ek ) ;
   }
 _current += ik;
...
#if NMODL_TEXT
static const char* nmodl_filename = "kas.mod";
static const char* nmodl_file_text = 
