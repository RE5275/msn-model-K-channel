...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "ghk_can", _hoc_ghk,
 "modulation_can", _hoc_modulation,
 "rates_can", _hoc_rates,
 0, 0
};
#define ghk ghk_can
#define modulation modulation_can
...
static int  rates ( _threadargsproto_ ) {
   ...
   mtau = ( 0.06 + 1.0 / ( exp ( ( v - 25.0 ) / 18.0 ) + exp ( ( v - ( - 31.0 ) ) / - 44.0 ) ) ) * 2.0 ;
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ica = pbar * m * m * ( h * a + 1.0 - a ) * ghk ( _threadargscomma_ v , cai , cao ) * modulation ( _threadargs_ ) ;
   }
 _current += ica;
...
#if NMODL_TEXT
static const char* nmodl_filename = "can.mod";
static const char* nmodl_file_text = 
