...
#define max2 _p[3]
#define max2_columnindex 3
#define level _p[4]
#define level_columnindex 4
#define lev2 _p[5]
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "modulation_km", _hoc_modulation,
 "rates_km", _hoc_rates,
 0, 0
};
#define modulation modulation_km
 extern double modulation( _threadargsproto_ );
 /* declare global and static user variables */
#define q10temp q10temp_km
 double q10temp = 22;
#define q10 q10_km
 double q10 = 2.3;
...
static int _reset;
static char *modelname = "Potassium current, M-type (I_K(M))";
...
static int  rates ( _threadargsprotocomma_ double _lVm ) {
   double _lvtau , _lktau1 , _lktau2 , _lalpha , _lbeta ;
 _lvtau = - 35.0 ;
   _lktau1 = 20.3 ;
   _lktau2 = - 20.3 ;
   _lalpha = 3.3 * exp ( ( _lVm - _lvtau ) / _lktau1 ) ;
   _lbeta = 3.3 * exp ( ( _lVm - _lvtau ) / _lktau2 ) ;
   taum = 1.0 / ( _lalpha + _lbeta ) * ( 1000.0 ) ;
   taum = taum / tadj ;
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {
  ...
  m = m0;
 {
   tadj = pow( q10 , ( ( celsius - q10temp ) / ( 10.0 ) ) ) ;
   rates ( _threadargscomma_ v ) ;
   m = minf ;
   }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   g = gbar * m * modulation ( _threadargs_ ) ;
   i = g * ( v - ek ) ;
   ik = i ;
   }
 _current += ik;
