...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "ghk_car", _hoc_ghk,
 "modulation_car", _hoc_modulation,
 "rates_car", _hoc_rates,
 0, 0
};
#define ghk ghk_car
#define modulation modulation_car
...
static int  rates ( _threadargsproto_ ) {
    minf = 1.0 / ( 1.0 + exp ( ( v - ( - 29.0 ) ) / ( - 9.6 ) ) ) ;
   mtau = 5.1 * 3.0 ;
   hinf = 1.0 / ( 1.0 + exp ( ( v - ( - 33.3 ) ) / 17.0 ) ) ;
   htau = 22.0 + 80.0 / ( 1.0 + exp ( ( v - ( - 19.0 ) ) / 5.0 ) ) ;
     return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ica = pbar * m * m * m * h * ghk ( _threadargscomma_ v , cai , cao ) * modulation ( _threadargs_ ) ;
   }
 _current += ica;
...
#if NMODL_TEXT
static const char* nmodl_filename = "car.mod";
static const char* nmodl_file_text = 
